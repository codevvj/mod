...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}Lỗi：${plain} Bạn Chưa Cấp Quyền Root！\n" && exit 1
...
if [[ -f /etc/redhat-release ]]; then
    ...
    echo -e "${red}Không kiểm tra thấy phiên bản hệ thống！${plain}\n" && exit 1
fi
...
install_base() {
    if [[ x"${release}" == x"centos" ]]; then
        ...
        yum install wget curl unzip tar crontabs socat nano htop -y
...
install_XrayR() {
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Sao Chép Cấu Hình Thành Công"
    else
